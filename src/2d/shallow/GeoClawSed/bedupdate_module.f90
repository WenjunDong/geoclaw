        ...
        module bedupdate_module
            ...
            subroutine bed_update(mbc,mx,my,t,dt,dx,dy,u,v,h,aux,naux)
                ...
                !local
                integer :: i,j,k
                Real(kind=Prec),dimension(1-mbc:mx+mbc,1-mbc:my+mbc) :: dzbdt,wet,sederold!,zb
                    ...
                    if (struct == 1) then
                        ...
                        do k = 1,gmax
                            do j=2,my+mbc
                                do i=2,mx+mbc
                                    !print *, i,j
                                    !print *, Susg(i,j,:)!*dy-Susg(i-1,j,:)*dy,'term1'
                                    !print *, Svsg(i,j,:)!*dx-Svsg(i,j-1,:)*dx ,'term2'
                                    !print *, Subg(i,j,:)!*dy-Subg(i-1,j,:)*dy, 'term3'
                                    !print *, Svbg(i,j,:)!*dx-Svbg(i,j-1,:)*dx,'term4'
                                    ! fluxes at i,j
                ...
                write (1,*),pbbed(4,:,1,1)
                !open (unit=1,status='replace',position='Append',file="pbbed5.txt",action="write")
                !write (1,*),pbbed(5,:,1,1)
                !close(1)
                !open (unit=1,status='replace',position='Append',file="pbbed6.txt",action="write")
                !write (1,*),pbbed(6,:,1,1)
                !close(1)
                !open (unit=1,status='replace',position='Append',file="pbbed7.txt",action="write")
                !write (1,*),pbbed(7,:,1,1)
                !close(1)
                !open (unit=1,status='replace',position='Append',file="pbbed8.txt",action="write")
                !write (1,*),pbbed(8,:,1,1)
                !close(1)
                if (t<1.0) then
